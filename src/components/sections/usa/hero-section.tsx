...
import { ArrowRight, Shield, Lock, Fingerprint, CheckCircle2 } from 'lucide-react';
import { useScrollAnimation } from '@/hooks/useScrollAnimation';
import { analytics } from '@/lib/analytics';
import Image from 'next/image';

export function USAHeroSection() {
    ...
    return (
        <section id="hero" ref={heroRef as React.RefObject<HTMLElement>} className="relative min-h-[90vh] flex flex-col justify-start pt-40 pb-12 px-4 sm:px-6 overflow-hidden">
            {/* Background Glows — red/blue US accent */}
                ...
                <div className={`flex flex-col items-center text-center max-w-4xl mx-auto ${heroVisible ? 'slide-up' : 'animate-on-scroll'}`}>
                    ...
                    <div className={`flex flex-wrap items-center justify-center gap-3 mb-8 ${heroVisible ? 'fade-in animate-delay-100' : 'animate-on-scroll'}`}>
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/10 border border-blue-500/30">
                            ...
                            <span className="text-sm font-semibold text-blue-400">The Silicon Valley Standard</span>
                        </div>
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-red-500/10 border border-red-500/30">
                            <Lock className="w-4 h-4 text-red-400" />
                            <span className="text-sm font-semibold text-red-400">SOC2 Type II Compliant</span>
                        </div>
                    </div>
                    ...
                    <h1 className={`text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-6 leading-[1.05] tracking-tight text-white ${heroVisible ? 'slide-up animate-delay-200' : 'animate-on-scroll'}`}>
                        Stop Hiring<br />
                        <span className="text-brand">AI Passengers.</span>
                    </h1>
                    ...
                    <p className={`text-lg sm:text-xl text-text-secondary mb-6 leading-relaxed max-w-2xl ${heroVisible ? 'slide-up animate-delay-300' : 'animate-on-scroll'}`}>
                        Gartner predicts 1 in 4 job candidates will be fake by 2028.
                        59% of US hiring managers already suspect AI misrepresentation.
                        Rigovo doesn&apos;t ban AI — we verify whether candidates <strong className="text-white">lead</strong> the AI or just <strong className="text-white">follow</strong> it.
                    </p>

                    {/* USA-specific value props */}
                    <div className={`flex flex-wrap justify-center gap-4 mb-10 ${heroVisible ? 'slide-up animate-delay-350' : 'animate-on-scroll'}`}>
                        {[
                            { icon: Fingerprint, text: 'Deepfake & proxy detection' },
                            { icon: Shield, text: '15-signal integrity verification' },
                            { icon: CheckCircle2, text: 'Legally-defensible audit trails' },
                        ].map((item, i) => (
                    ...
                    <div className={`flex flex-col items-center gap-4 ${heroVisible ? 'slide-up animate-delay-400' : 'animate-on-scroll'}`}>
                        ...
                        >
                            <span className="relative z-10 technical-label text-sm">See Verified Results</span>
                        ...
                        <p className="text-text-muted text-sm">
                            Free pilot — 90 minutes of integrity-verified interviews
                        </p>
                    </div>
                    ...
                    <div className={`mt-16 relative w-full max-w-4xl ${heroVisible ? 'slide-up animate-delay-500' : 'animate-on-scroll'}`}>
                        <div className="relative rounded-[2rem] overflow-hidden border border-white/10 bg-card/30 backdrop-blur-md shadow-2xl">
                            <Image
                                src="/assets/maya.jpg"
                                alt="Maya AI interviewer verifying candidate integrity for US enterprise technical hiring"
                                width={1200}
                                height={675}
                            ...
                            {/* Overlay Stats — US-specific framing */}
                            <div className="absolute bottom-6 left-6 right-6 flex flex-wrap items-center justify-center gap-4 sm:gap-8">
                                {[
                                    { value: '$240K', label: 'Max cost of a bad hire (SHRM)' },
                                    { value: '1 in 4', label: 'Candidates will be fake by 2028' },
                                    { value: '59%', label: 'Managers suspect AI misuse' },
                                ].map((stat, i) => (
                        ...
                        {/* Decorative elements */}
                        <div className="absolute -top-8 -right-8 w-48 h-48 bg-blue-500/20 blur-[80px] rounded-full -z-10" />
                        <div className="absolute -bottom-8 -left-8 w-48 h-48 bg-red-500/10 blur-[80px] rounded-full -z-10" />
                    </div>
                </div>
            </div>
        </section>
